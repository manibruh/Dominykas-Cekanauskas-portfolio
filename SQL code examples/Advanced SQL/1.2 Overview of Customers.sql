...
-- Final query, selects only top 200 active customers (have placed an order in the last year)
-- Ordered by Total amount with tax
SELECT *
FROM latest_top_customers
WHERE latest_top_customers.LastOrderDate < recent_order - INTERVAL 365 DAY
ORDER BY latest_top_customers.TotalAmountWithTax DESC
LIMIT 200